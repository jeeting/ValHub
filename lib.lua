...
do
    ...
    function Funcs:AddToggle(Idx, Info)
        ...
        local KnobCorner = Library:Create('UICorner', {
            CornerRadius = UDim.new(0, 7),
            Parent = ToggleKnob,
        })

        -- Toggle label with modern styling
...
function Library:SetWatermark(Text)
    if not Library.WatermarkEnabled then
        return -- Don't update if watermark is disabled
    end
    
    if Library.Watermark and Library.WatermarkText then
...
function Library:SetKeybindListVisibility(Bool)
    Library.KeybindListEnabled = Bool
...
function Library:Notify(Text, Time, Type)
    ...
    local TypeColors = {
        info = Library.AccentColor,
        success = Library.SuccessColor,
        warning = Library.WarningColor,
        error = Library.RiskColor
    }
    
    local AccentColor = TypeColors[Type] or Library.AccentColor
    ...
    -- Improved animation
    NotifyOuter:TweenSize(UDim2.new(0, XSize + 16, 0, YSize), 'Out', 'Quart', 0.3, true)

    task.spawn(function()
        task.wait(Time)
        NotifyOuter:TweenSize(UDim2.new(0, 0, 0, YSize), 'Out', 'Quart', 0.3, true)
        task.wait(0.3)
        NotifyOuter:Destroy()
    end)
end
...
function Library:CreateWindow(...)
    ...
    if type(Config.Title) ~= 'string' then Config.Title = 'Vel Hub V2' end
    ...
    -- Main outer frame with shadow effect
    local Outer = Library:Create('Frame', {
    ...
    -- Main window frame with modern styling
    local Inner = Library:Create('Frame', {
        BackgroundColor3 = Library.MainColor,
        BorderSizePixel = 0,
        Position = UDim2.new(0, 1, 0, 1),
    ...
    -- Add subtle gradient
    local MainGradient = Library:Create('UIGradient', {
        Color = ColorSequence.new({
    ...
    Library:AddToRegistry(MainGradient, {
        Color = function()
            return ColorSequence.new({
                ColorSequenceKeypoint.new(0, Library.MainColor),
                ColorSequenceKeypoint.new(1, Library:GetDarkerColor(Library.MainColor)),
    ...
    Library:AddToRegistry(TitleFix, {
        BackgroundColor3 = 'AccentColor',
    })

    -- Window title with better styling
    local WindowLabel = Library:CreateLabel({
        Position = UDim2.new(0, 12, 0, 0),
        Size = UDim2.new(1, -50, 1, 0),
        Text = Config.Title or 'Vel Hub V2',
        TextXAlignment = Enum.TextXAlignment.Left,
        TextSize = 16,
    ...
    CloseButton.MouseEnter:Connect(function()
        Library:CreateTween(CloseButton, TweenInfo.new(0.2), {BackgroundColor3 = Color3.fromRGB(255, 80, 80)})
    end)

    CloseButton.MouseLeave:Connect(function()
        Library:CreateTween(CloseButton, TweenInfo.new(0.2), {BackgroundColor3 = Color3.fromRGB(237, 66, 69)})
    end)

    CloseButton.MouseButton1Click:Connect(function()
        Library:CreateTween(Outer, TweenInfo.new(0.3, Enum.EasingStyle.Quart), {Size = UDim2.new(0, 0, 0, 0)})
        task.wait(0.3)
        Outer:Destroy()
    end)
    ...
    -- Add subtle border to tab container
    local TabBorder = Library:Create('UIStroke', {
    ...
    Library:AddToRegistry(TabBorder, {
        Color = 'OutlineColor',
    })
    ...
    function Window:AddTab(Name)
        ...
        -- Modern tab button with rounded corners
        ...
        -- Tab button label with better styling
        ...
        TabButton.MouseEnter:Connect(function()
            if not Tab.Active then
                Library:CreateTween(TabButton, TweenInfo.new(0.2), {BackgroundColor3 = Library:GetDarkerColor(Library.BackgroundColor)})
            end
        end)

        TabButton.MouseLeave:Connect(function()
            if not Tab.Active then
                Library:CreateTween(TabButton, TweenInfo.new(0.2), {BackgroundColor3 = Library.BackgroundColor})
            end
        end)
        ...
        -- Left side with modern scrolling
        ...
        -- Right side with modern scrolling
        ...
        function Tab:AddGroupbox(Info)
            ...
            -- Modern groupbox with rounded corners and shadow
            ...
            -- Fix bottom corners of highlight
            local HighlightFix = Library:Create('Frame', {
                BackgroundColor3 = Library.AccentColor,
                BorderSizePixel = 0,
            ...
            Library:AddToRegistry(HighlightFix, {
                BackgroundColor3 = 'AccentColor',
            })

            -- Enhanced groupbox label
